...
PVOID get_system_module_export( LPCWSTR module_name, LPCSTR routine_name )
{
	...
	return asd2abnnSDF::RtlFindExportedRoutineByName( lpModule, routine_name );
}
...
{
	ULONG bytes = 0;
	asd2abnnSDF::ZwQuerySystemInformation( SystemModuleInformation, 0, bytes, &bytes );
	...
	if ( !NT_SUCCESS(asd2abnnSDF::ZwQuerySystemInformation( SystemModuleInformation, modules, bytes, &bytes ) ) )
	{
		ExFreePool( modules );
		return nullptr;
	}
